...
def perform_cv(X_train,y_train,regressor,param_grid,cv,Time_res_dir,):
    ...
    # Saving the objects:
    f = open(Time_res_dir+'/best_model_NOnested.pckl', 'wb')
    pickle.dump(best_model, f)
    f.close()


def save_cv_results(Time_res_dir):
...
def ComputeSave_preverr(X_prev,y_prev,Time_res_dir):
...
def perform_lc(X,y,Time_res_dir):
    ...
    # Saving the objects:
    f = open(Time_res_dir+'/best_model_lc.pckl', 'wb')
    pickle.dump(best_model, f)
    f.close()

def save_lc_results(Time_res_dir):

  reader = open(Time_res_dir+'/best_model_lc.pckl', 'rb')
  ...
  display_r2.plot()
  plt.savefig(Time_res_dir+'/Figures/LC_r2.png', dpi=300)
  #plt.show()

  with open(Time_res_dir+'/Resultat.txt', 'a') as f:
        f.write("\n\nReusult Learning Curve MGMT\n")
...
def compute_feature_importances_MDI(regressor,feature_names,Time_res_dir):
    ...
    f = open(Time_res_dir+'/FI_MDI.pckl', 'wb')
    pickle.dump(reg_importances_sorted, f)
    f.close()


def compute_feature_importances_SHAP(regressor,X_train,X_test,Time_res_dir):
     # Step 5: SHAP for Variable Importance
     explainer = shap.Explainer(regressor, X_train)
     shap_values = explainer.shap_values(X_test)
     print(shap_values)
     f = open(Time_res_dir+'/FI_SH.pckl', 'wb')
